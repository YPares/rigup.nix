...
- Enable riglets to add MCP servers to the rig, through the harness config generated by `entrypoint` riglets (ongoing, see `mcp` branch)
- Enable riglets to add custom "slash" commands ("prompt templates") to the rig, through similar mechanism, for harnesses that support it (see `prompt-cmds` branch)
