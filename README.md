...
- **Plug-and-play integration with common agent harnesses** via riglets defining an `entrypoint` derivation. `rigup` will generate config for:
  - Claude Code
  - OpenCode
...
- **Configure more than just Skills:** Harnesses that support it will be fed config for:
  - `mcpServers`: riglets can define MCP servers, both HTTP and command-based
  - `promptCommands`: riglets can define custom "slash commands" (prompt templates)
- **Reproducible:** Nix ensures consistent tool versions
- **Data-driven config:** `rigup.toml` for CLI-manageable rigs. Riglets from `riglets/` automatically exposed
